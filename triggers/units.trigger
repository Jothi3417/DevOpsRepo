trigger units on Order__c (after insert,after update,after delete,after undelete) {
    ...
    if(trigger.isinsert || trigger.isundelete){
        ...
        Map<id,E_Product__c> prodMap = new Map<id, E_Product__c>([select id,Units_Available__c from E_Product__c where id in :idList]);
        for(Order__c oc : Trigger.new )
        {
            E_Product__c prod = prodMap.get(oc.Product__c);
    ...
    Map<id,E_Product__c> proMap = new Map<id, E_Product__c>([select id,Units_Available__c from E_Product__c where id in :idList]);
    for(Order__c itr1 : Trigger.old )
    {
    E_Product__c pro = proMap.get(itr1.Product__c);
    ...
    Map<id,E_Product__c> productmap = new Map<id, E_Product__c>([select id,Units_Available__c from E_Product__c where id in :updateid]);
    for(Order__c itr3 : orderList)
    {
    E_Product__c pr = productMap.get(itr3.Product__c);
